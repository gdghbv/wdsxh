        ...
        if ($this->request->isPost()) {
            ...
            foreach ($memberList as $rowIndex => $row) {
                ...
                for ($j = count($person_fieldset); $j < $company_count; $j++) {
                    ...
                    if (isset($row['custom_content']['company'][$key])) {
                        if($row['custom_content']['company'][$key]['type'] == 'cert'){
                            $value = !empty($row['custom_content']['company']['value']) ? wdsxh_full_url($row['custom_content']['company'][$key]['value']['image']) : '';
                        } elseif ($row['custom_content']['company'][$key]['field'] == 'address') {
                            $value = !empty($row['custom_content']['company'][$key]['value']) ? $row['custom_content']['company'][$key]['value']['address'] : '';
                        } else {
                            $value = isset($row['custom_content']['company'][$key]['value']) ? $row['custom_content']['company'][$key]['value'] : '';
                            if (in_array($row['custom_content']['company'][$key]['type'],['image','video'])) {
                                $value = wdsxh_full_url($value);
                                if (is_array($value)) {
                                    $value = implode(',',$value);
                                }
                            }
                            if ($row['custom_content']['company'][$key]['field'] == 'member_level_id') {
                                $value = $levelModel->where('id',$value)->value('name');
                            }
                            if ($row['custom_content']['company'][$key]['field'] == 'industry_category_id') {
                                $value = $industryModel->where('id',$value)->value('name');
                            }
                        }
                    }
