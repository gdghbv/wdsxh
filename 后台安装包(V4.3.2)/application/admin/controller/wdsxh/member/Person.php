    ...
    public function export($ids = "")
    {
        if ($this->request->isPost()) {
            ...
            foreach ($memberList as $rowIndex => $row) {
                ...
                for ($j = 0; $j < count($row['custom_content']); $j++) {
                    if($row['custom_content'][$j]['type'] == 'cert'){
                        $value = !empty($row['custom_content'][$j]['value']) ? wdsxh_full_url($row['custom_content'][$j]['value']['image']) : '';
                    } elseif ($row['custom_content'][$j]['field'] == 'address') {
                        $value = !empty($row['custom_content'][$j]['value']) ? $row['custom_content'][$j]['value']['address'] : '';
                    } else {
                        $value = isset($row['custom_content'][$j]['value']) ? $row['custom_content'][$j]['value'] : '';
                        if (in_array($row['custom_content'][$j]['type'],['image','video'])) {
                            $value = wdsxh_full_url($value);
                            if (is_array($value)) {
                                $value = implode(',',$value);
                            }
                        }
                        if ($row['custom_content'][$j]['field'] == 'member_level_id') {
                            $value = $levelModel->where('id',$value)->value('name');
                        }
                        if ($row['custom_content'][$j]['field'] == 'industry_category_id') {
