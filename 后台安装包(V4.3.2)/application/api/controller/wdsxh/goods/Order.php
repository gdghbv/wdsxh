...
class Order extends Api
{
    ...
    public function create()
    {
        ...
        if ($buy_now == 1) {
            //直接购买，查询单个商品
            //查询积分
            $goods = (new \app\api\model\wdsxh\goods\Goods())->where('id', $param['goods_id'])->find();
            $goods_price = $goods['price'];
            $goods_points = $goods['points'] ?? 0;
            ...
            $param['points_earned'] = $goods_points * $param['number'];//计算该次获取的积分总量
            $result = false;
            Db::startTrans();
            try {
            ...
            /*  $goodsList =(new \app\common\model\Goods())
             ->where('id','in',$goods_id_array)
             ->column('id,price,name,image,points','id'); */
            foreach ($cartList as $v) {
                /* $goods=$goodList[$v['goods_id']]??null;
                if(!$goodsObj){continue;} */
                ...
                $item_points = ($goodsObj['points'] ?? 0) * $v['goods_num'];
            ...
            $param['createtime'] = time();
            $param['delivery_method'] = $delivery_method;
            $param['points_earned'] = $total_points;
            $result = false;
            Db::startTrans();
            try {
                $result = $this->model->allowField(true)->save($param);
    ...
    public function details()
    {
        ...
        $row = $this->model
            ...
            trade_no,
            buy_now,
            delivery_method,pick_up_code,points_earned')
            ->find();
        if (!$row) {
            $this->error('订单不存在');
        }
    ...
    public function payResult()
    {
        ...
        $response = $pay->handlePaidNotify(function ($message, $fail) {
            ...
            $wechatObj = (new \app\api\model\wdsxh\user\Wechat())->where('id', $order['wechat_id'])->find();
            $pointsLogObj = \think\Db::name('wdsxh_user_wechat_points_log');
            Db::startTrans();
            try {
                $order->pay_time = time();
                $order->state = '2';
                ...
                $order->save();
                //付款成功，将积分添加到微信用户表中并更新
                $total_points = $points_earned + $wechatObj['total_points'];
                $points = $points_earned + $wechatObj['points'];
                $wechatObj->total_points = $total_points;
                $wechatObj->points = $points;
                //将积分记录表添加新的积分记录
                $pointsLogData = [
                    'wechat_id' => $order['wechat_id'],//用户id
                    'points' => $points_earned,//变更积分
                    'before' => $wechatObj['points'],//变更前积分
                    'after' => $points,//变更后积分
                    'total_points' => $total_points,//累计获得积分
                    'memo' => '订单支付获得积分',//备注
                    'createtime' => time(),//创建时间
                    'change' => 1,//类型 1=订单支付获得积分
                    'source' => 2,
                ];
                $pointsLogObj->insert($pointsLogData);
                $wechatObj->save();
                Db::commit();
            } catch (\think\Exception $e) {
                Db::rollback();
    ...
    public function refund()
    {
        ...
        if ($orderModel['state'] == '2') {
            ...
            if ($res && $res['return_code'] == 'SUCCESS' && $res['result_code'] == 'SUCCESS') {
                Db::startTrans();
                try {
                    $orderObj=(new \app\api\model\wdsxh\goods\Order())->where('id', $param['order_id']);
                    $orderObj->update([
                            'state' => '-2',
                            'refund_status' => '5',
                            'complete_time' => time(),
                    ...
                    //退款成功扣除相应的积分
                    $wechatObj=(new \app\api\model\wdsxh\user\Wechat())->where('id',$wechat_id)->find();
                    $pointsLogObj=\think\Db::name('wesxh_user_wechat_points_log');
                    $wechatObj->points = $wechatObj['points'] -$orderObj['points_earned'];
                    $wechatObj->total_points = $wechatObj['total_points'] -$orderObj['points_earned'];
                    $pointsLogData=[
                        'wechat_id'=>$orderObj['wechat_id'],
                        'points'=>$orderObj['points_earned'],//变更积分
                        'before'=>$wechatObj['points'],//变更前积分
                        'after'=>$wechatObj['points'] -$orderObj['points_earned'],//变更后积分
                        'total_points'=>$wechatObj['total_points'] -$orderObj['points_earned'],//累计获得积分
                        'memo'=>'订单退款扣除积分',//备注
                        'createtime'=>time(),//创建时间
                        'change'=>2,//类型 1=订单支付获得积分,2=订单退款
                        'source'=>2,
                    ];
                    $pointsLogObj->insert($pointsLogData);
                    $wechatObj->save();
